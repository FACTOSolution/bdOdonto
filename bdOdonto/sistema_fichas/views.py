...
class CadastroView(View):
    ...
    template_post_nome = None

    def get(self, request, *args, **kwargs):
        form = self.form_class()
        return render(request, self.template_get_nome, {'form': form})
    
    def post(self, request, form):
        if form.is_valid():
            form.save()
            return HttpResponseRedirect(reverse(self.template_post_nome))
        else:
            return render(request, self.template_get_nome, {'form':self.form_class})

class CadastrarPaciente(CadastroView):
    form_class = PacienteForm
    template_get_nome = 'sistema_fichas/cadastrar_paciente.html'
    template_post_nome = 'sistema_fichas:menu_paciente'
    
    def post(self, request, *args, **kwargs):   
        formPaciente = PacienteForm(request.POST, request.FILES)
        return super().post(request,formPaciente)

class CadastrarPlanejamento(CadastroView):
    form_class = PlanejamentoForm
    template_get_nome = 'sistema_fichas/cadastrar_planejamento.html'
    template_post_nome = 'sistema_fichas:listar_planejamentos'
    ...
    def post(self, request, *args, **kwargs):
        ...
        return super().post(request,plan)

class CadastrarProcedimento(CadastroView):
    form_class = ProcedimentoForm
    template_get_nome = 'sistema_fichas/cadastrar_procedimento.html'
    template_post_nome = 'sistema_fichas:listar_planejamentos'
